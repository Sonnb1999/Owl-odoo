...
class TestProjectReport(TestProjectCommon):
    def test_avg_rating_measure(self):
        ...
        task_3 = self.env['th.project.task'].create({
            'name': 'task 3',
        ...
        self.env['th.project.task'].invalidate_model(['rating_avg', 'rating_last_value'])

        tasks = [self.task_1, self.task_2, task_3]
        for task in tasks:
            rating_values = task.read(['rating_avg', 'rating_last_value'])[0]
            task_report = self.env['th.report.project.task.user'].search_read([('project_id', '=', self.project_pigs.id), ('task_id', '=', task.id)], ['rating_avg', 'rating_last_value'])[0]
