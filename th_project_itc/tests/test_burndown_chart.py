...
class TestBurndownChartCommon(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        Stage = cls.env['th.project.task.type']
        ...
        cls.project = cls.env['th.project.project'].create({
        ...
        cls.set_create_date('th_project_project', cls.project.id, create_date)
        cls.project.invalidate_model()
        cls.milestone = cls.env['th.project.milestone'].with_context({'mail_create_nolog': True}).create({
            'name': 'Test Milestone',
            'project_id': cls.project_pigs.id,
        })
        cls.task_a = cls.env['th.project.task'].create({
            'name': 'Task A',
            'priority': 0,
            'project_id': cls.project.id,
            'stage_id': cls.todo_stage.id,
        })
        cls.set_create_date('th_project_task', cls.task_a.id, create_date)
        cls.task_b = cls.task_a.copy({
            'name': 'Task B',
            'user_ids': [Command.set([cls.user_projectuser.id, cls.user_projectmanager.id])],
        })
        cls.set_create_date('th_project_task', cls.task_b.id, create_date)
        cls.task_c = cls.task_a.copy({
            'name': 'Task C',
            'partner_id': cls.partner_1.id,
            'user_ids': [Command.link(cls.user_projectuser.id)],
        })
        cls.set_create_date('th_project_task', cls.task_c.id, create_date)
        cls.task_d = cls.task_a.copy({
            'name': 'Task D',
            'milestone_id': cls.milestone.id,
            'user_ids': [Command.link(cls.user_projectmanager.id)],
        })
        cls.set_create_date('th_project_task', cls.task_d.id, create_date)
        cls.task_e = cls.task_a.copy({
            'name': 'Task E',
            'partner_id': cls.partner_1.id,
        })
        cls.set_create_date('th_project_task', cls.task_e.id, create_date)

        # Create a new task to check if a task without changing its stage is taken into account
        task_f = cls.env['th.project.task'].create({
            'name': 'Task F',
            'priority': 0,
            'project_id': cls.project.id,
            'milestone_id': cls.milestone.id,
        ...
        cls.project_2 = cls.env['th.project.project'].create({
            'name': 'Burndown Chart Test 2 mySearchTag',
            'privacy_visibility': 'employees',
            'alias_name': 'project+burndown_chart+2',
            'type_ids': [Command.link(stage_id) for stage_id in cls.stages.ids],
        })
        cls.set_create_date('th_project_project', cls.project_2.id, create_date)
        cls.project.invalidate_model()
        cls.task_g = cls.env['th.project.task'].create({
            'name': 'Task G',
            'priority': 0,
...
class TestBurndownChart(TestBurndownChartCommon):
    ...
    def check_read_group_results(self, domain, expected_results_dict):
        ...
        read_group_result = self.env['th.project.task.burndown.chart.report'].read_group(
