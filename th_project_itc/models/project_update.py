...
class ProjectUpdate(models.Model):
    ...
    def default_get(self, fields):
        ...
        if result.get('project_id'):
            project = self.env['th.project.project'].browse(result['project_id'])
    ...
    project_id = fields.Many2one('th.project.project', required=True)
    ...
    def _build_description(self, project):
        return self.env['ir.qweb']._render('th_project_itc.project_update_default_description', self._get_template_values(project))
    ...
    def _get_milestone_values(self, project):
        Milestone = self.env['th.project.milestone']
    ...
    def _get_last_updated_milestone(self, project):
        query = """
            SELECT DISTINCT pm.id as milestone_id,
                 ...
                 INNER JOIN ir_model_fields imf
                         ON mtv.field = imf.id
                        AND imf.model = 'th.project.milestone'
                        AND imf.name = 'deadline'
                 INNER JOIN th_project_milestone pm
                         ON mm.res_id = pm.id
                      WHERE mm.model = 'th.project.milestone'
        ...
        milestones = self.env['th.project.milestone'].search([('id', 'in', list(mapped_result.keys()))])
        return [{
            **milestone._get_data(),
