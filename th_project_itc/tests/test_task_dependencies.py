...
class TestTaskDependencies(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        cls.task_3 = cls.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
    ...
    def test_tracking_dependencies(self):
        ...
        self.env['res.config.settings'].create({
            'th_group_project_task_dependencies': True
        }).execute()
    ...
    def test_task_dependencies_settings_change(self):
        ...
        def set_task_dependencies_setting(enabled):
            features_config = self.env["res.config.settings"].create({'th_group_project_task_dependencies': enabled})
        ...
        self.project_chickens = self.env['th.project.project'].create({
        ...
        self.project_ducks = self.env['th.project.project'].create({
    ...
    def test_duplicate_project_with_task_dependencies(self):
        ...
        task1_copy = pigs_copy.th_task_ids.filtered(lambda t: t.name == 'Pigs UserTask')
        task2_copy = pigs_copy.th_task_ids.filtered(lambda t: t.name == 'Pigs ManagerTask')
        ...
        task1_copy_copy = pigs_copy_copy.th_task_ids.filtered(lambda t: t.name == 'Pigs UserTask')
    ...
    def test_duplicate_project_with_subtask_dependencies(self):
        ...
        parent_task = self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Parent Task',
