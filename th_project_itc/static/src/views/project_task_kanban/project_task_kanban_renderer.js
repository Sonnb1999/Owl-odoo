...
export class ThProjectTaskKanbanRenderer extends KanbanRenderer {
    setup() {
        super.setup();
        ...
        // đóng trên kanban
        // this.props.list.groups[0].isFolded = true;
    }
    ...
    async onWillStart() {
        if (!this.props.list.isGroupedByPersonalStages) { // no need to check it if the group by is personal stages
            this.isProjectManager = await this.userService.hasGroup('th_project_itc.th_group_project_manager');
        }
    }
    ...
    editGroup(group) {
        ...
        const context = Object.assign({}, group.context, {
            form_view_ref: 'th_project_itc.th_personal_task_type_edit',
        });
