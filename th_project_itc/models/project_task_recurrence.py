...
class ProjectTaskRecurrence(models.Model):
    _name = 'th.project.task.recurrence'
    _description = 'Task Recurrence'

    th_task_ids = fields.One2many('th.project.task', 'recurrence_id', copy=False)
    ...
    def _create_subtasks(self, task, new_task, depth=3):
        ...
        for child in task.child_ids:
            ...
            if child.child_ids and depth > 1:
                ...
                # "grandchildren" tasks
                new_child = self.env['th.project.task'].sudo().create(child_values)
                self._create_subtasks(child, new_child, depth=depth - 1)
            else:
                children.append(child_values)
        self.env['th.project.task'].sudo().create(children)

    def _create_next_task(self):
        for recurrence in self:
            task = recurrence.sudo().th_task_ids[-1]
            create_values = recurrence._new_task_values(task)
            new_task = self.env['th.project.task'].sudo().create(create_values)
    ...
    def _cron_create_recurring_tasks(self):
        if not self.env.user.has_group('th_project_itc.th_group_project_recurring_tasks'):
            return
        today = fields.Date.today()
