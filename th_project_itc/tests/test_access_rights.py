...
class TestAccessRights(TestProjectCommon):
    ...
    def create_task(self, name, *, with_user=None, **kwargs):
        ...
        return self.env['th.project.task'].with_user(with_user or self.env.user).create(values)
...
class TestCRUDVisibilityFollowers(TestAccessRights):
    ...
    def test_project_no_unlink(self):
        self.project_pigs.th_task_ids.unlink()
...
class TestProjectPortalCommon(TestProjectCommon):
    ...
    def setUp(self):
        ...
        self.task_3 = self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Test3', 'user_ids': self.user_portal, 'project_id': self.project_pigs.id})
        self.task_4 = self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Test4', 'user_ids': self.user_public, 'project_id': self.project_pigs.id})
        self.task_5 = self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Test5', 'user_ids': False, 'project_id': self.project_pigs.id})
        self.task_6 = self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Test5', 'user_ids': False, 'project_id': self.project_pigs.id})
...
class TestPortalProject(TestProjectPortalCommon):
    ...
    def test_employee_project_access_rights(self):
        ...
        tasks = self.env['th.project.task'].with_user(self.user_projectuser).search([('project_id', '=', pigs.id)])
        ...
        tmp_task = self.env['th.project.task'].with_user(self.user_projectuser).with_context({'mail_create_nolog': True}).create({
            'name': 'Pigs task',
            'project_id': pigs.id})
    ...
    def test_followers_project_access_rights(self):
        ...
        self.env['th.project.task'].with_user(self.user_projectuser).with_context({'mail_create_nolog': True}).create({
        ...
        self.assertRaises(AccessError, self.env['th.project.task'].with_user(self.user_projectuser).with_context({
...
class TestAccessRightsPrivateTask(TestAccessRights):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.private_task = cls.env['th.project.task'].create({'name': 'OdooBot Private Task'})
    ...
    def create_private_task(self, name, with_user=None, **kwargs):
        ...
        return self.env['th.project.task'].with_user(user).create(values)
    ...
    def test_project_current_user_is_added_in_private_task_assignees(self):
        ...
        my_private_task = self.env['th.project.task'].create(task_values)
        ...
        user_projectuser_private_task = self.env['th.project.task'].create({**task_values, 'user_ids': [Command.set(self.user_projectuser.ids)]})
    ...
    def test_project_current_user_is_added_in_task_assignees_when_project_id_is_set(self):
        ...
        user_projectuser_task = self.env['th.project.task'].create(task_values)
    ...
    def test_project_current_user_is_set_as_assignee_in_task_when_project_id_is_set_with_no_assignees(self):
        task = self.env['th.project.task'].create({'name': 'Private task', 'project_id': self.project_pigs.id})
    ...
    def test_project_current_user_is_not_added_in_private_task_assignees_when_default_project_id_is_in_the_context(self):
        ...
        ProjectTask_with_default_project_id = self.env['th.project.task'].with_context(context)
