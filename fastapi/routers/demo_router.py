...
@router.get("/partner")
async def partner(env: Environment = Depends(odoo_env)):
    partners = env['res.partner'].sudo().search([])
    return [{'name': partner.name} for partner in partners]
...
async def who_ami(partner: Annotated[Partner, Depends(authenticated_partner)]) -> DemoUserInfo:
    """Who am I?

    Returns the authenticated partner
    """
    # This method show you how you can get the authenticated partner without
