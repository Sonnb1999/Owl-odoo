...
import { ThProjectStopRecurrenceConfirmationDialog } from '../components/project_stop_recurrence_confirmation_dialog/project_stop_recurrence_confirmation_dialog';

class ThProjectTaskRecurrence {
    constructor(env, dialog, orm) {
        this.env = env;
        this.dialog = dialog;
        this.orm = orm;
        this.resModel = 'th.project.task';
    }
    ...
    async stopRecurrence(tasks, callback = () => {}) {
        ...
        this.dialog.add(ThProjectStopRecurrenceConfirmationDialog, dialogProps);
    }
}

export const thtaskRecurrenceService = {
    dependencies: ['dialog', 'orm'],
    async: [
        'stopRecurrence',
    ],
    start(env, { dialog, orm }) {
        return new ThProjectTaskRecurrence(env, dialog, orm);
    }
};

registry.category('services').add('th_project_task_recurrence', thtaskRecurrenceService);
