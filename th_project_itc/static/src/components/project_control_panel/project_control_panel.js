...
export class ThProjectControlPanel extends ControlPanel {
    setup() {
        super.setup();
    ...
    async loadData() {
        const [data, isProjectUser] = await Promise.all([
            this.orm.call("th.project.project", "get_last_update_or_default", [this.projectId]),
            this.user.hasGroup("th_project_itc.th_group_project_user"),
        ]);
        this.data = data;
    ...
    async onStatusClick(ev) {
        ev.preventDefault();
        this.actionService.doAction("th_project_itc.th_project_update_all_action", {
            additionalContext: {
...
ThProjectControlPanel.template = "th_project_itc.ProjectControlPanel";
