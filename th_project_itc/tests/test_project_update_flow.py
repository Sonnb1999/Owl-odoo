...
class TestProjectUpdate(TestProjectCommon):
    ...
    def test_project_update_form(self):
        with Form(self.env['th.project.milestone'].with_context({'default_project_id': self.project_pigs.id})) as milestone_form:
            milestone_form.name = "Test 1"
            milestone_form.deadline = fields.Date.today()

        try:
            with Form(self.env['th.project.update'].with_context({'default_project_id': self.project_pigs.id})) as update_form:
    ...
    def test_project_update_description(self):
        ...
        template_values = self.env['th.project.update']._get_template_values(self.project_pigs)
