        ...
        if (hasChanged) {
            if (changed.changed.component) {
                ...
                if (component.properties.hasOwnProperty('change')) {
                    ...
                    if (changeOverlay) {
                        // Thay đổi: Trả về Promise và thêm vào hàng đợi
                        return new Promise((resolve) => {
                            // Tạo overlay timer
                            let overlayTimerPromise = new Promise((timerResolve) => {
                                window.setTimeout(timerResolve, changeOverlay);
                            });
                            
                            self.showOverlay();
                            
                            // Thêm request vào hàng đợi
                            self.promiseQueue = self.promiseQueue.then(() => {
                                return this.postData(apiUrl, data).then(function(result) {
                                    ...
                                    return overlayTimerPromise.then(() => {
                                        self.hideOverlay();
                                        resolve();
                                    });
                                });
    ...
    onBlur(form, instance) {
        ...
        if (instance.component) {
            ...
            if (component.properties.hasOwnProperty('blur')) {
                ...
                if (blurOverlay) {
                    // Thêm vào hàng đợi với overlay timer
                    return new Promise((resolve) => {
                        let overlayTimerPromise = new Promise((timerResolve) => {
                            window.setTimeout(timerResolve, blurOverlay);
                        });
                        ...
                        return self.promiseQueue;
                         ...
                         && !!self.params['overlay_api_change'])
                {
                    // Thêm vào hàng đợi với overlay từ tham số
                    return new Promise((resolve) => {
                        self.showOverlay();
                    ...
                    // Thêm vào hàng đợi không có overlay
                    return new Promise((resolve) => {
                        self.promiseQueue = self.promiseQueue.then(() => {
                            return this.postData(apiUrl, data).then(function(result) {
                                if (result.hasOwnProperty('values')) {
    ...
    createForm() {
        ...
        Formio.createForm(document.getElementById('formio_form'), self.schema, self.options).then(function(form) {
            ...
            form.on('submit', function(submission) {
                if (form.element.getElementsByClassName('btn btn-md mt-2').length !== 0) {
                    form.element.getElementsByClassName('btn btn-md mt-2')[0].classList.add('disabled')
                }

                if (form.element.getElementsByClassName('btn mt-2').length !== 0) {
                    form.element.getElementsByClassName('btn mt-2')[0].classList.add('disabled')
                }

                if (form.element.getElementsByClassName('th_disabled').length !== 0) {
                    form.element.getElementsByClassName('th_disabled')[0].classList.add('disabled')
                }
                // get_url in parent_url
                let utmParams = {};
                let affiliate_in_form = ''
                let full_url = window.location.href
                // full_url = "https://samaff.eln.vn/formio/public/form/new/5606e8d5-5750-4018-b65e-90cfdd5011a8?referrer=https%3A%2F%2Fl.facebook.com%2F&utm_source=hand&utm_campaign=chiendich1206&utm_medium=google_cpc&utm_content=contentquangcaoso1&utm_term=doituongquangcaoso1&full_url=https%3A%2F%2Fsonnb1999.github.io%2Fnoithat1%2F%3Futm_source%3Dhand%26utm_medium%3Dgoogle_cpc%26utm_campaign%3Dchiendich1206%26utm_term%3Ddoituongquangcaoso1%26utm_content%3Dcontentquangcaoso1%26fbclid%3DIwZXh0bgNhZW0CMTAAAR18oyqhcWF0TaehPRh8U4V8bliGp0SN_4l0oVQOmroloPtfzh7P8ZQl90A_aem__2vExIHZfFM0A-r2o2hAUQ"
                ...
                submission.data.th_url = full_url
                submission.data.th_utm = utmParams
