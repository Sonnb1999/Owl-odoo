...
def _check_exists_collaborators_for_project_sharing(env):
    ...
    """
    collaborator = env['th.project.collaborator'].search([], limit=1)
    if collaborator:
        # Then we need to enable the access rights linked to project sharing for the portal user
        env['th.project.collaborator']._toggle_project_sharing_portal_rules(True)
...
def _project_post_init(cr, registry):
    ...
    project_task_stage_field_id = env['ir.model.fields']._get_ids('th.project.task').get('stage_id')
    create_index(
        cr,
