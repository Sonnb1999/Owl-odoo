...
class TestProjectSharingCommon(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        project_sharing_stages_vals_list = [
            ...
            (0, 0, {'name': 'Done', 'sequence': 10, 'fold': True, 'rating_template_id': cls.env.ref('th_project_itc.th_rating_project_request_email_template').id}),
        ]
        ...
        cls.project_cows = cls.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'Cows',
            'privacy_visibility': 'portal',
            'alias_name': 'project+cows',
            'type_ids': project_sharing_stages_vals_list,
        })
        cls.project_portal = cls.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'Portal',
            'privacy_visibility': 'portal',
        ...
        cls.task_cow = cls.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Cow UserTask',
            'user_ids': cls.user_projectuser,
            'project_id': cls.project_cows.id,
        })
        cls.task_portal = cls.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Portal UserTask',
            'user_ids': cls.user_projectuser,
            'project_id': cls.project_portal.id,
        })

        cls.project_sharing_form_view_xml_id = 'th_project_itc.th_project_sharing_project_task_view_form'
...
class TestProjectSharing(TestProjectSharingCommon):
    ...
    def test_project_share_wizard(self):
        ...
        """
        project_share_wizard = self.env['th.project.share.wizard'].create({
            'res_model': 'th.project.project',
    ...
    def test_create_task_in_project_sharing(self):
        ...
        with self.assertRaises(AccessError, msg="Should not accept the portal user create a task in the project when he has not the edit access right."):
            with self.get_project_sharing_form_view(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_portal.id}), self.user_portal) as form:
                form.name = 'Test'
                task = form.save()
        ...
        with self.get_project_sharing_form_view(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_portal.id, 'default_user_ids': [(4, self.user_portal.id)]}), self.user_portal) as form:
            form.name = 'Test'
            task = form.save()
    ...
    def test_edit_task_in_project_sharing(self):
        ...
        task2 = self.env['th.project.task'] \
            .with_context({
    ...
    def test_orm_method_with_true_false_domain(self):
        ...
        task = self.env['th.project.task'].with_user(self.user_portal).search(
            expression.AND([
        ...
        task_read_group = self.env['th.project.task'].read_group(
