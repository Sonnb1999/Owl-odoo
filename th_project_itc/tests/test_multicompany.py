...
class TestMultiCompanyProject(TestMultiCompanyCommon):
    ...
    def setUpClass(cls):
        ...
        Project = cls.env['th.project.project'].with_context({'mail_create_nolog': True, 'tracking_disable': True})
        ...
        Task = cls.env['th.project.task'].with_context({'mail_create_nolog': True, 'tracking_disable': True})
    ...
    def test_create_project(self):
        ...
        with self.sudo('manager-a'):
            project = self.env['th.project.project'].with_context({'tracking_disable': True}).create({
                'name': 'Project Company A',
    ...
    def test_create_subtask(self):
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                with Form(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_parent_id': self.task_1.id, 'default_project_id': self.project_company_b.id})) as task_form:
                ...
                view = self.env.ref('th_project_itc.th_view_task_form2').sudo()
