...
class TestProjectCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        user_group_project_user = cls.env.ref('th_project_itc.th_group_project_user')
        user_group_project_manager = cls.env.ref('th_project_itc.group_project_manager')
        ...
        # Test 'Pigs' project
        cls.project_pigs = cls.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'Pigs',
            'privacy_visibility': 'employees',
            'alias_name': 'project+pigs',
            'partner_id': cls.partner_1.id})
        # Already-existing tasks in Pigs
        cls.task_1 = cls.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Pigs UserTask',
            'user_ids': cls.user_projectuser,
            'project_id': cls.project_pigs.id})
        cls.task_2 = cls.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Pigs ManagerTask',
            'user_ids': cls.user_projectmanager,
            'project_id': cls.project_pigs.id})

        # Test 'Goats' project, same as 'Pigs', but with 2 stages
        cls.project_goats = cls.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'Goats',
    ...
    def format_and_process(self, template, to='groups@example.com, other@gmail.com', subject='Frogs',
                           ...
                           model=None, target_model='th.project.task', target_field='name'):
...
class TestProjectBase(TestProjectCommon):
    ...
    def test_delete_project_with_tasks(self):
        ...
        task_type = self.env['th.project.task.type'].create({'name': 'Won', 'sequence': 1, 'fold': True})
        project_unlink = self.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'rev',
            'privacy_visibility': 'employees',
            'alias_name': 'rev',
        ...
        th_task_count = len(project_unlink.tasks)
        self.assertEqual(th_task_count, 1, "The project should have 1 task")

        project_unlink.unlink()
        self.assertNotEqual(th_task_count, 0, "The all tasks linked to project should be deleted when user delete the project")
    ...
    def test_auto_assign_stages_when_importing_tasks(self):
        ...
        self.env['th.project.task']._load_records_create([{
            'name': 'First Task',
    ...
    def test_filter_visibility_unread_messages(self):
        ...
        for user, filter_invisible_expected in ((user1, True), (user2, None)):
            Task = self.env['th.project.task'].with_user(user)
            arch = Task.get_view(self.env.ref('th_project_itc.th_view_task_search_form').id, 'search')['arch']
