...
class ProjectSharingChatter(PortalChatter):
    def _check_project_access_and_get_token(self, project_id, res_model, res_id, token):
        ...
        """
        project_sudo = ProjectCustomerPortal._document_check_access(self, 'th.project.project', project_id, token)
        can_access = project_sudo and res_model == 'th.project.task' and project_sudo.with_user(request.env.user)._check_project_sharing_access()
        task = None
        if can_access:
            task = request.env['th.project.task'].sudo().search([('id', '=', res_id), ('project_id', '=', project_sudo.id)])
