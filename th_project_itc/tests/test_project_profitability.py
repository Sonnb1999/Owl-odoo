...
class TestProjectProfitabilityCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.project = cls.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'Project',
            'partner_id': cls.partner.id,
            'analytic_account_id': cls.analytic_account.id,
        })
        cls.task = cls.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Task',
