...
class TestProjectMilestone(TestProjectCommon):
    ...
    def test_milestones_settings_change(self):
        ...
        self.env['res.config.settings'] \
            .create({'th_group_project_milestone': False}) \
            .execute()
        self.assertFalse(self.env.user.has_group('th_project_itc.th_group_project_milestone'), 'The "Milestones" feature should not be globally enabled by default.')
        self.assertFalse(self.project_pigs.allow_milestones, 'The "Milestones" feature should not be enabled by default.')
        self.env['res.config.settings'] \
            .create({'th_group_project_milestone': True}) \
            .execute()
        self.assertTrue(self.env.user.has_group('th_project_itc.th_group_project_milestone'), 'The "Milestones" feature should globally be enabled.')
        ...
        project = self.env['th.project.project'].create({'name': 'Test allow_milestones on New Project'})
        ...
        with Form(self.env['th.project.project']) as project_form:
