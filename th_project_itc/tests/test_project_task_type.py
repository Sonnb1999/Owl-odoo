...
class TestProjectTaskType(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        cls.stage_created = cls.env['th.project.task.type'].create({
            'name': 'Stage Already Created',
        })

    def test_create_stage(self):
        '''
        Verify that it is not possible to add to a newly created stage a `user_id` and a `th_project_ids`
        '''
        with self.assertRaises(UserError):
            self.env['th.project.task.type'].create({
                'name': 'New Stage',
                'user_id': self.uid,
                'th_project_ids': [self.project_goats.id],
            })
