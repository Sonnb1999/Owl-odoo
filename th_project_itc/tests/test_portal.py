...
class TestPortalProject(TestProjectPortalCommon):
    ...
    def test_portal_project_access_rights(self):
        ...
        self.assertRaises(AccessError, self.env['th.project.task'].with_user(self.user_noone).search, [('project_id', '=', pigs.id)])
        ...
        self.assertRaises(AccessError, self.env['th.project.task'].with_user(self.user_public).search, [])
