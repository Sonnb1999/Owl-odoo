...
class TestPersonalStages(TestProjectCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.user_stages = cls.env['th.project.task.type'].search([('user_id', '=', cls.user_projectuser.id)])
        cls.manager_stages = cls.env['th.project.task.type'].search([('user_id', '=', cls.user_projectmanager.id)])
    ...
    def test_personal_stage_base(self):
        ...
        self.assertFalse(self.env['th.project.task'].browse(self.task_1.id).with_user(self.user_projectmanager).personal_stage_type_id,
    ...
    def test_personal_stage_search(self):
        ...
        tasks = self.env['th.project.task'].with_user(self.user_projectuser).search([('personal_stage_type_id', '=', self.user_stages[0].id)])
    ...
    def test_personal_stage_read_group(self):
        ...
        self.env.flush_all()
        read_group_user = self.env['th.project.task'].with_user(self.user_projectuser).read_group(
        ...
        read_group_manager = self.env['th.project.task'].with_user(self.user_projectmanager).read_group(
