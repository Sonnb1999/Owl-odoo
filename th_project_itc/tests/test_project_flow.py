...
class TestProjectFlow(TestProjectCommon, MailCommon):
    ...
    def test_task_process_without_stage(self):
        ...
        self.assertEqual(task.message_ids.subtype_id, self.env.ref('th_project_itc.th_mt_task_new'),
    ...
    def test_subtask_process(self):
        ...
        Task = self.env['th.project.task'].with_context({'tracking_disable': True})
    ...
    def test_rating(self):
        ...
        rating_good = self.env['rating.rating'].create({
            'res_model_id': self.env['ir.model']._get('th.project.task').id,
            'res_id': first_task.id,
            'parent_res_model_id': self.env['ir.model']._get('th.project.project').id,
    ...
    def test_task_with_no_project(self):
        ...
        """
        task_without_project = self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
    ...
    def test_send_rating_review(self):
        project_settings = self.env["res.config.settings"].create({'th_group_project_rating': True})
        ...
        rating_request_mail_template = self.env.ref('th_project_itc.th_rating_project_request_email_template')
        won_stage.write({'rating_template_id': rating_request_mail_template.id})
        tasks = self.env['th.project.task'].with_context(mail_create_nolog=True, default_project_id=self.project_goats.id).create([
    ...
    def test_email_track_template(self):
        ...
        project_settings = self.env["res.config.settings"].create({'th_group_project_stages': True})
        project_settings.execute()
        ...
        mail_template = self.env['mail.template'].create({
            ...
            'body_html': '<p>Test</p>',
            'auto_delete': True,
            'model_id': self.env.ref('th_project_itc.model_th_project_project_stage').id,
        })
        project_A = self.env['th.project.project'].create({
            'name': 'project_A',
        ...
        project_stage = self.env.ref('th_project_itc.project_project_stage_1')
