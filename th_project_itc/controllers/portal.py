...
class ProjectCustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'th_project_count' in counters:
            values['th_project_count'] = request.env['th.project.project'].search_count([]) \
                if request.env['th.project.project'].check_access_rights('read', raise_exception=False) else 0
        if 'th_task_count' in counters:
            values['th_task_count'] = request.env['th.project.task'].search_count([]) \
                if request.env['th.project.task'].check_access_rights('read', raise_exception=False) else 0
        return values
    ...
    def portal_my_projects(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        Project = request.env['th.project.project']
        ...
        # projects count
        th_project_count = Project.search_count(domain)
        # pager
        pager = portal_pager(
            url="/my/projects",
            url_args={'date_begin': date_begin, 'date_end': date_end, 'sortby': sortby},
            total=th_project_count,
            page=page,
    ...
    def portal_my_project(self, project_id=None, access_token=None, page=1, date_begin=None, date_end=None, sortby=None, search=None, search_in='content', groupby=None, task_id=None, **kw):
        try:
            project_sudo = self._document_check_access('th.project.project', project_id, access_token)
    ...
    def _prepare_project_sharing_session_info(self, project, task=None):
        ...
        session_info.update(
            ...
            action_name='th_project_itc.th_project_sharing_project_task_action',
    ...
    def render_project_backend_view(self, project_id, task_id=None):
        project = request.env['th.project.project'].sudo().browse(project_id)
        if not project.exists() or not project.with_user(request.env.user)._check_project_sharing_access():
            return request.not_found()
        task = task_id and request.env['th.project.task'].browse(int(task_id))
        return request.render(
            'th_project_itc.project_sharing_embed',
    ...
    def portal_my_project_task(self, project_id=None, task_id=None, access_token=None, **kw):
        try:
            ...
            return request.redirect('/my')
        Task = request.env['th.project.task']
        if access_token:
            Task = Task.sudo()
    ...
    def portal_my_project_subtasks(self, project_id, task_id, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='content', groupby=None, **kw):
        try:
            project_sudo = self._document_check_access('th.project.project', project_id)
            task_sudo = request.env['th.project.task'].search([('project_id', '=', project_id), ('id', '=', task_id)]).sudo()
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        if project:
            ...
            try:
                project_accessible = bool(task.project_id.id and self._document_check_access('th.project.project', task.project_id.id))
            except (AccessError, MissingError):
    ...
    def _get_my_tasks_searchbar_filters(self, project_domain=None, task_domain=None):
        ...
        projects = request.env['th.project.project'].search(project_domain or [])
