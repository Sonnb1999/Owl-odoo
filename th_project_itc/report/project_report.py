...
class ReportProjectTaskUser(models.Model):
    _name = "th.report.project.task.user"
    _description = "Tasks Analysis"
    _order = 'name desc, project_id'
    _auto = False

    name = fields.Char(string='Task', readonly=True)
    user_ids = fields.Many2many('res.users', relation='th_project_task_user_rel', column1='task_id', column2='user_id',
    ...
    project_id = fields.Many2one('th.project.project', string='Project', readonly=True)
    ...
    rating_last_value = fields.Float('Rating Value (/5)', group_operator="avg", readonly=True, groups="th_project_itc.th_group_project_rating")
    rating_avg = fields.Float('Average Rating', readonly=True, group_operator='avg', groups="th_project_itc.th_group_project_rating")
    ...
    stage_id = fields.Many2one('th.project.task.type', string='Stage', readonly=True)
    is_closed = fields.Boolean("Closing Stage", readonly=True, help="Folded in Kanban stages are closing stages.")
    task_id = fields.Many2one('th.project.task', string='Tasks', readonly=True)
    active = fields.Boolean(readonly=True)
    tag_ids = fields.Many2many('th.project.tags', relation='th_project_tags_th_project_task_rel',
        column1='project_task_id', column2='project_tags_id',
        string='Tags', readonly=True)
    parent_id = fields.Many2one('th.project.task', string='Parent Task', readonly=True)
    ancestor_id = fields.Many2one('th.project.task', string="Ancestor Task", readonly=True)
    ...
    personal_stage_type_ids = fields.Many2many('th.project.task.type', relation='th_project_task_user_rel',
        column1='task_id', column2='stage_id',
        string="Personal Stage", readonly=True)
    milestone_id = fields.Many2one('th.project.milestone', readonly=True)
        ...
        return f"""
                th_project_task t
                    LEFT JOIN rating_rating rt ON rt.res_id = t.id
                        AND rt.res_model = 'th.project.task'
