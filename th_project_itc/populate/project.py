...
class ProjectProject(models.Model):
    _inherit = "th.project.project"
    _populate_sizes = {"small": 10, "medium": 50, "large": 1000}
    _populate_dependencies = ["res.company", "th.project.task.type"]

    def _populate_factories(self):
        company_ids = self.env.registry.populated_models["res.company"]
        stage_ids = self.env.registry.populated_models["th.project.task.type"]
...
class ProjectTask(models.Model):
    _inherit = "th.project.task"
    _populate_sizes = {"small": 500, "medium": 5000, "large": 50000}
    _populate_dependencies = ["th.project.project"]

    def _populate_factories(self):
        th_project_ids = self.env.registry.populated_models["th.project.project"]
        stage_ids = self.env.registry.populated_models["th.project.task.type"]
        def get_project_id(random, **kwargs):
            return random.choice([False, False, False] + th_project_ids)
    ...
    def _populate_set_children_tasks(self, tasks, size):
        ...
        tasks -= parents
        parent_childs = collections.defaultdict(lambda: self.env['th.project.task'])
