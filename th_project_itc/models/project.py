...
class ProjectTaskType(models.Model):
    ...
    th_project_ids = fields.Many2many('th.project.project', 'th_project_task_type_rel', 'type_id', 'project_id', string='Projects',
    ...
    is_itc_stage = fields.Boolean(default=False)
    is_other_tasks_stage = fields.Boolean(default=False)
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    th_check_stage = fields.Selection(selection=[('done', 'Học liệu đạt'), ('up', 'Up hệ thống')], string='Giai đoạn')
    is_completed_state = fields.Boolean(string='Là giai đoạn hoàn thành',default=False)
    ...
    def unlink_wizard(self, stage_view=False):
        ...
        readgroup = self.with_context(active_test=False).env['th.project.task']._read_group([('stage_id', 'in', self.ids)], ['project_id'], ['project_id'])
        th_project_ids = list(set([project['project_id'][0] for project in readgroup] + self.th_project_ids.ids))

        wizard = self.with_context(th_project_ids=th_project_ids).env['th.project.task.type.delete.wizard'].create({
            'th_project_ids': th_project_ids,
            'stage_ids': self.ids
        })
        ...
        return {
            'name': _('Delete Stage'),
            'view_mode': 'form',
            'res_model': 'th.project.task.type.delete.wizard',
            'views': [(self.env.ref('th_project_itc.th_view_project_task_type_delete_wizard').id, 'form')],
    ...
    def write(self, vals):
        if 'active' in vals and not vals['active']:
            self.env['th.project.task'].search([('stage_id', 'in', self.ids)]).write({'active': False})
        return super(ProjectTaskType, self).write(vals)

    def toggle_active(self):
        res = super().toggle_active()
        stage_active = self.filtered('active')
        inactive_tasks = self.env['th.project.task'].with_context(active_test=False).search(
            [('active', '=', False), ('stage_id', 'in', stage_active.ids)], limit=1)
        if stage_active and inactive_tasks:
            wizard = self.env['th.project.task.type.delete.wizard'].create({
            ...
            return {
                ...
                'view_mode': 'form',
                'res_model': 'th.project.task.type.delete.wizard',
                'views': [(self.env.ref('th_project_itc.th_view_project_task_type_unarchive_wizard').id, 'form')],
                'type': 'ir.actions.act_window',
                'res_id': wizard.id,
                'target': 'new',
            }
    ...
    @api.depends('th_project_ids', 'th_project_ids.rating_active')
    def _compute_disabled_rating_warning(self):
        for stage in self:
            disabled_projects = stage.th_project_ids.filtered(lambda p: not p.rating_active)
            if disabled_projects:
                stage.disabled_rating_warning = '\n'.join('- %s' % p.name for p in disabled_projects)
            else:
                stage.disabled_rating_warning = False

    # @api.constrains('user_id', 'th_project_ids')
    # def _check_personal_stage_not_linked_to_projects(self):
    #     if any(stage.user_id and stage.th_project_ids for stage in self):
    ...
    def remove_personal_stage(self):
        ...
        users_personal_stages = self.env['th.project.task.type']\
        ...
        new_stage = self.env['th.project.task.type']
        ...
        self.env['th.project.task.stage.personal'].search([('stage_id', '=', self.id)]).write({
            'stage_id': new_stage.id,
        })
        self.unlink()

class Project(models.Model):
    _name = "th.project.project"
        ...
        self.env.cr.execute(
            ...
            WITH docs AS (
                 SELECT res_id as id, count(*) as count
                   FROM ir_attachment
                  WHERE res_model = 'th.project.project'
                    AND res_id IN %(th_project_ids)s
               GROUP BY res_id
              ...
              UNION ALL
                 ...
                 SELECT t.project_id as id, count(*) as count
                   ...
                   JOIN th_project_task t ON a.res_model = 'th.project.task' AND a.res_id = t.id
                  WHERE t.project_id IN %(th_project_ids)s
    ...
    def _compute_th_task_count(self):
        ...
        task_all_data = self.env['th.project.task'].with_context(active_test=False)._read_group(domain, fields, groupby)
    ...
    def _inverse_is_favorite(self):
        favorite_projects = not_fav_projects = self.env['th.project.project'].sudo()
        for project in self:
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        return self.env['th.project.project.stage'].search([('company_id','=', self.env.company.id)], order=order)
    ...
    favorite_user_ids = fields.Many2many(
        'res.users', 'th_project_favorite_user_rel', 'project_id', 'user_id',
    ...
    tasks = fields.One2many('th.project.task', 'project_id', string="Task Activities")
    resource_calendar_id = fields.Many2one(
        'resource.calendar', string='Working Time',
        related='company_id.resource_calendar_id')
    type_ids = fields.Many2many('th.project.task.type', 'th_project_task_type_rel', 'project_id', 'type_id', string='Tasks Stages')
    th_task_count = fields.Integer(compute='_compute_th_task_count', string="Task Count")
    task_count_with_subtasks = fields.Integer(compute='_compute_th_task_count')
    th_task_ids = fields.One2many('th.project.task', 'project_id', string='Tasks',
    ...
    allow_subtasks = fields.Boolean('Sub-tasks', default=lambda self: self.env.user.has_group('th_project_itc.th_group_subtask_project'))
    allow_recurring_tasks = fields.Boolean('Recurring Tasks', default=lambda self: self.env.user.has_group('th_project_itc.th_group_project_recurring_tasks'))
    allow_task_dependencies = fields.Boolean('Task Dependencies', default=lambda self: self.env.user.has_group('th_project_itc.th_group_project_task_dependencies'))
    allow_milestones = fields.Boolean('Milestones', default=lambda self: self.env.user.has_group('th_project_itc.th_group_project_milestone'))
    tag_ids = fields.Many2many('th.project.tags', relation='th_project_project_th_project_tags_rel', string='Tags')
    task_properties_definition = fields.PropertiesDefinition('Task Properties')

    # Project Sharing fields
    collaborator_ids = fields.One2many('th.project.collaborator', 'project_id', string='Collaborators', copy=False)
    collaborator_count = fields.Integer('# Collaborators', compute='_compute_collaborator_count', compute_sudo=True)

    # rating fields
    rating_request_deadline = fields.Datetime(compute='_compute_rating_request_deadline', store=True)
    rating_active = fields.Boolean('Customer Ratings', default=lambda self: self.env.user.has_group('th_project_itc.th_group_project_rating'))
    allow_rating = fields.Boolean('Allow Customer Ratings', compute="_compute_allow_rating", default=lambda self: self.env.user.has_group('th_project_itc.th_group_project_rating'))
    ...
    stage_id = fields.Many2one('th.project.project.stage', string='Stage', ondelete='restrict',
        tracking=True, index=True, copy=False, default=_default_stage_id, group_expand='_read_group_stage_ids')

    update_ids = fields.One2many('th.project.update', 'project_id')
    last_update_id = fields.Many2one('th.project.update', string='Last Update', copy=False)
    ...
    milestone_ids = fields.One2many('th.project.milestone', 'project_id', copy=True)
    milestone_count = fields.Integer(compute='_compute_milestone_count', groups='th_project_itc.th_group_project_milestone')
    milestone_count_reached = fields.Integer(compute='_compute_milestone_reached_count', groups='th_project_itc.th_group_project_milestone')
    is_milestone_exceeded = fields.Boolean(compute="_compute_is_milestone_exceeded", search='_search_is_milestone_exceeded')
    th_link_sheet = fields.Html("Link sheet")
    th_member_ids = fields.Many2many(comodel_name="res.users", relation="th_member_project_rel", column1="th_project_id",
                                     column2="th_member_id", string="Người tham gia",
                                     domain=lambda self: [('id', 'in', self.env.ref("th_project_itc.th_group_project_user").users.ids)])
    th_is_check = fields.Boolean('Báo cáo', copy=False)
    th_is_pay = fields.Boolean('Thanh toán', copy=False)
    ...
    def _compute_allow_rating(self):
        self.allow_rating = self.env.user.has_group('th_project_itc.th_group_project_rating')
    ...
    def _compute_milestone_count(self):
        read_group = self.env['th.project.milestone']._read_group([('project_id', 'in', self.ids)], ['project_id'], ['project_id'])
    ...
    def _compute_milestone_reached_count(self):
        read_group = self.env['th.project.milestone']._read_group(
    ...
    def _search_is_milestone_exceeded(self, operator, value):
        ...
        query = """
            SELECT P.id
              FROM th_project_project P
         LEFT JOIN th_project_milestone M ON P.id = M.project_id
    ...
    def _compute_collaborator_count(self):
        ...
        collaborator_read_group = self.env['th.project.collaborator']._read_group(
    ...
    def map_tasks(self, new_project_id):
        ...
        new_task_ids = []
        new_subtasks = self.env['th.project.task']
        # We want to copy archived task, but do not propagate an active_test context key
        th_task_ids = self.env['th.project.task'].with_context(active_test=False).search([('project_id', '=', self.id), ('parent_id', '=', False)]).ids
        if self.allow_task_dependencies and 'task_mapping' not in self.env.context:
            self = self.with_context(task_mapping=dict())
        for task in self.env['th.project.task'].browse(th_task_ids):
    ...
    def name_create(self, name):
        ...
        if res:
            ...
            self.browse(res[0]).type_ids += self.env['th.project.task.type'].sudo().create({'name': _('New')})
        return res
    ...
    def write(self, vals):
        ...
        if 'last_update_status' in vals and vals['last_update_status'] != 'to_define':
            for project in self:
                ...
                self.env['th.project.update'].with_context(default_project_id=project.id).create({
        ...
        if 'allow_recurring_tasks' in vals and not vals.get('allow_recurring_tasks'):
            self.env['th.project.task'].search([('project_id', 'in', self.ids), ('recurring_task', '=', True)]).write({'recurring_task': False})

        if 'active' in vals:
            # archiving/unarchiving a project does it on its tasks, too
            self.with_context(active_test=False).mapped('tasks').write({'active': vals['active']})
        if 'name' in vals and self.analytic_account_id:
            projects_read_group = self.env['th.project.project']._read_group(
    ...
    def _alias_get_creation_values(self):
        ...
        values['alias_model_id'] = self.env['ir.model']._get('th.project.task').id
        if self.id:
    ...
    def _track_template(self, changes):
        ...
        project = self[0]
        if self.user_has_groups('th_project_itc.th_group_project_stages') and 'stage_id' in changes and project.stage_id.mail_template_id:
    ...
    def _track_subtype(self, init_values):
        ...
        if 'stage_id' in init_values:
            return self.env.ref('th_project_itc.th_mt_project_stage_change')
        return super()._track_subtype(init_values)

    def _mail_get_message_subtypes(self):
        res = super()._mail_get_message_subtypes()
        if len(self) == 1:
            dependency_subtype = self.env.ref('th_project_itc.th_mt_project_task_dependency_change')
    ...
    def action_project_task_burndown_chart_report(self):
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_action_project_task_burndown_chart_report')
    ...
    def project_update_all_action(self):
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_project_update_all_action')
    ...
    def action_view_tasks(self):
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_act_project_project_2_project_task_all')
    ...
    def action_view_link_sheet(self):
        action = self.env['ir.actions.actions']._for_xml_id('th_project_itc.th_project_link_sheet_action')
        action['context'] = {'default_th_project_id': self.id}
        return action

    def action_view_all_rating(self):
        """ return the action to see all the rating of the project and activate default filters"""
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_rating_rating_action_view_project_rating')
        ...
        action['domain'] = [('consumed', '=', True), ('parent_res_model', '=', 'th.project.project'), ('parent_res_id', '=', self.id)]
    ...
    def action_view_tasks_analysis(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_action_project_task_user_tree')
    ...
    def action_get_list_view(self):
        ...
        return {
            ...
            'res_model': 'th.project.milestone',
            'views': [(self.env.ref('th_project_itc.th_project_milestone_view_tree').id, 'tree')],
            'view_mode': 'tree',
            'help': _("""
    ...
    def get_panel_data(self):
        self.ensure_one()
        if not self.user_has_groups('th_project_itc.th_group_project_user'):
            return {}
        panel_data = {
    ...
    def get_milestones(self):
        if self.user_has_groups('th_project_itc.th_group_project_user'):
    ...
    def _get_user_values(self):
        return {
            'is_project_user': self.user_has_groups('th_project_itc.th_group_project_user'),
        }
    ...
    def _get_stat_buttons(self):
        ...
        buttons = [{
            'icon': 'tasks',
            'text': _lt('Tasks'),
            'number': self.th_task_count,
            'action_type': 'action',
            'action': 'th_project_itc.th_act_project_project_2_project_task_all',
        ...
        if self.rating_count != 0 and self.user_has_groups('th_project_itc.th_group_project_rating'):
        ...
        if self.user_has_groups('th_project_itc.th_group_project_user'):
            buttons.append({
                ...
                'action_type': 'action',
                'action': 'th_project_itc.th_action_project_task_burndown_chart_report',
                'additional_context': json.dumps({
                    'active_id': self.id,
                }),
                'show': True,
            ...
            buttons.append({
                ...
                'action_type': 'action',
                'action': 'th_project_itc.th_project_collaborator_action',
                'additional_context': json.dumps({
                    'active_id': self.id,
                }),
    ...
    def _create_analytic_account_from_values(self, values):
        ...
        analytic_account = self.env['account.analytic.account'].create({
            ...
            'plan_id': company.th_analytic_plan_id.id,
        })
    ...
    def _create_analytic_account(self):
        for project in self:
            analytic_account = self.env['account.analytic.account'].create({
                ...
                'plan_id': project.company_id.th_analytic_plan_id.id,
                'active': True,
            })
    ...
    def _send_rating_all(self):
        ...
        for project in projects:
            project.th_task_ids._send_task_rating_mail()
    ...
    def _change_privacy_visibility(self, new_visibility):
        ...
        for project in self:
            ...
            if new_visibility == 'portal':
                ...
                for task in project.th_task_ids.filtered('partner_id'):
...
class Task(models.Model):
    ...
    # def _get_domain_user_project(self):
    #     project_id = self.env.context.get('default_project_id')
    #     if not project_id:
    #         return False
    #     return [('id', 'in',0)]
    ...
    def _default_personal_stage_type_id(self):
        return self.env['th.project.task.type'].search([('user_id', '=', self.env.user.id)], limit=1).id

    @api.model
    def _default_company_id(self):
        if self._context.get('default_project_id'):
            return self.env['th.project.project'].browse(self._context['default_project_id']).company_id
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        ...
        if 'default_project_id' in self.env.context:
            search_domain = ['|', ('th_project_ids', '=', self.env.context['default_project_id'])] + search_domain

        stage_ids = stages._search(search_domain, order=order, access_rights_uid=SUPERUSER_ID)
        return stages.browse(stage_ids)

    # def _default_domain_user_ids(self):
    #     users_itc = self.env.ref("th_project_itc.th_group_project_user").users.ids
    #     return [('id', 'in', users_itc)]
    ...
    active = fields.Boolean(default=True)
    th_planned_hours = fields.Float("Giờ phân bổ", tracking=True, copy=False)
    ...
    stage_id = fields.Many2one('th.project.task.type', string='Stage', compute='_compute_stage_id',
        store=True, readonly=False, ondelete='restrict', tracking=True, index=True,
        default=_get_default_stage_id, group_expand='_read_group_stage_ids',
        domain="[('th_project_ids', '=', project_id)]", copy=False, task_dependency_tracking=True)
    tag_ids = fields.Many2many('th.project.tags', string='Tags',
    ...
    date_deadline = fields.Date(string='Deadline', index=True, copy=False, task_dependency_tracking=True, compute="_compute_date_deadline", store=True)
    ...
    project_id = fields.Many2one('th.project.project', string='Project', recursive=True,
    ...
    display_project_id = fields.Many2one('th.project.project', index=True)
    ...
    user_ids = fields.Many2many('res.users', relation='th_project_task_user_rel', column1='task_id', column2='user_id', string='Assignees', context={'active_test': False},domain="[('id', 'parent_of', th_project_partner_ids)]", tracking=True,)
    ...
    personal_stage_type_ids = fields.Many2many('th.project.task.type', 'th_project_task_user_rel', column1='task_id', column2='stage_id',
    ...
    personal_stage_id = fields.Many2one('th.project.task.stage.personal', string='Personal Stage State', compute_sudo=False,
    ...
    personal_stage_type_id = fields.Many2one('th.project.task.type', string='Personal User Stage',
    ...
    partner_id = fields.Many2one('res.partner',
        string='Người quản lý', recursive=True, tracking=True,
    ...
    displayed_image_id = fields.Many2one('ir.attachment', domain="[('res_model', '=', 'th.project.task'), ('res_id', '=', id), ('mimetype', 'ilike', 'image')]", string='Cover Image')
    ...
    parent_id = fields.Many2one('th.project.task', string='Parent Task', index=True)
    ancestor_id = fields.Many2one('th.project.task', string='Ancestor Task', compute='_compute_ancestor_id', index='btree_not_null', recursive=True, store=True)
    child_ids = fields.One2many('th.project.task', 'parent_id', string="Sub-tasks")
    ...
    # Tracking of this field is done in the write function
    depend_on_ids = fields.Many2many('th.project.task', relation="th_task_dependencies_rel", column1="task_id",
                                     column2="depends_on_id", string="Blocked By", tracking=True, copy=False,
                                     domain="[('project_id', '!=', False), ('id', '!=', id)]")
    dependent_ids = fields.Many2many('th.project.task', relation="th_task_dependencies_rel", column1="depends_on_id",
    ...
    recurrence_id = fields.Many2one('th.project.task.recurrence', copy=False)
    ...
    th_finish_date = fields.Datetime("Ngày hoàn thành", copy=False, compute='_compute_th_finish_date')
    th_difficulty = fields.Char("Độ khó", copy=False)

    # th_stage_domain = fields.Char(compute="_compute_th_stage_domain")
    # th_stage_domain = fields.Char(compute="_compute_th_stage_domain")
    th_id_stage = fields.Char("id trang thai",store=True)
    th_is_working = fields.Selection([('start', 'Bắt đầu'), ('stop', 'Dừng'), ('resume', 'Tiếp tục'), ('end', 'Kết thúc')], 'Nút chấm công', default='start')
    th_time_spent = fields.Float("Thời gian đã dùng")
    th_is_other_tasks = fields.Boolean("Nhiệm vụ khác", default=False)
    th_project_team_ids = fields.Many2many(comodel_name="th.project.team", relation="th_project_team_rel", column1="task_id", column2="team_id", string="Đội", compute="_compute_th_project_team_id", store=True)
    th_date_program = fields.Date(string="Ngày bắt đầu lập trình", store=True)
    th_date_start_sx = fields.Date(string="Ngày bắt đầu sx thô")
    th_date_end_sx = fields.Date(string="Ngày kết thúc sx thô")
    th_date_acceptance = fields.Date(string="Ngày nghiệm thu")
    th_date_start_input = fields.Date(string="Ngày nhập liệu")
    th_date_end_input = fields.Date(string="Ngày hoàn thành nhập liệu")
    th_date_complete_program = fields.Date(string="Ngày hoàn thành lập trình", store=True)
    th_date_test = fields.Date(string="Ngày bắt đầu Test", store=True)
    th_date_complete_test = fields.Date(string="Ngày hoàn thành Test", store=True)
    th_project_partner_ids = fields.Many2many(comodel_name="res.users",string="aaaa", related="project_id.th_member_ids")
    th_user_test_ids = fields.Many2many(comodel_name="res.users", relation="th_user_test_rel", column1="th_project_id" ,column2="th_user_test_id", string="Người kiểm thử")
    th_user_acceptance_ids = fields.Many2many(comodel_name="res.users",relation="th_user_acceptance_ids", string="Người nghiệm thu HL thô",domain="[('id', 'parent_of', th_project_partner_ids)]")
    th_user_input_ids = fields.Many2many(comodel_name="res.users", string="Người nhập liệu",relation="th_user_input_ids", domain="[('id', 'parent_of', th_project_partner_ids)]")
    th_user_acceptance_input_ids = fields.Many2many(comodel_name="res.users",relation="th_user_acceptance_input_ids", string="Người nghiệm thu nhập liệu",domain="[('id', 'parent_of', th_project_partner_ids)]")
    th_is_bug = fields.Boolean(string="Là Bug", default=False, copy=False)
    th_execution_time = fields.Float(string="Giờ thực hiện")
    th_is_check = fields.Boolean(string="Báo cáo", related="project_id.th_is_check", store=True)
    th_date_deadline_ids = fields.One2many("th.date.deadline", "th_project_task_id", string="Tiến độ", copy=False)
    th_date_deadline = fields.Date("Hạn chót giai đoạn", copy=False, compute="_compute_date_deadline")
    th_date_done_courseware = fields.Date('Ngày học liệu đạt', copy=False)
    th_date_up_courseware = fields.Date('Ngày up hệ thống', copy=False)
    th_return_stage_done = fields.Boolean('Trả về học liệu', copy=False, default=False)
    th_return_stage_up = fields.Boolean('Không up hệ thống', copy=False, default=False)
    th_domain_partner = fields.Char(default=[], compute="_compute_th_domain_partner")
    th_date_order = fields.Date(string="Ngày đặt hàng", copy=False, tracking=True)
    th_is_pay = fields.Boolean(string="Yêu cầu thanh toán", related="project_id.th_is_pay", store=True)
    th_type_raw_material = fields.Selection(selection=[('pending', 'Chờ duyệt'), ('approved', 'Duyệt'), ('cancel', 'Huỷ')], string="Thanh toán học liệu thô", copy=False, tracking=True)
    th_type_input_data = fields.Selection(selection=[('pending', 'Chờ duyệt'), ('approved', 'Duyệt'), ('cancel', 'Huỷ')], string="Thanh toán nhập liệu", copy=False, tracking=True)
    th_date_raw_materials_approved = fields.Date(string="Ngày xác nhận thanh toán học liệu thô", copy=False, tracking=True)
    th_date_input_data_approved = fields.Date(string="Ngày xác nhận thanh toán nhập liệu", copy=False, tracking=True)
    th_is_check_again = fields.Boolean(string="Yêu cầu kiểm tra lại", copy=False)
    th_expiration_date = fields.Datetime(string="Ngày hết hạn", copy=False, tracking=True)

    def th_action_approve(self):
        for rec in self:
            rec.th_is_check_again = False
            if rec.th_type_raw_material == 'pending':
                rec.th_type_raw_material = 'approved'
                rec.th_date_raw_materials_approved = fields.date.today()
            if rec.th_type_input_data == 'pending':
                rec.th_type_input_data = 'approved'
                rec.th_date_input_data_approved = fields.date.today()

    def th_action_cancel(self):
        for rec in self:
            rec.th_is_check_again = False
            if rec.th_type_raw_material == 'pending':
                rec.th_type_raw_material = 'cancel'
                rec.th_date_raw_materials_approved = False

            if rec.th_type_input_data == 'pending':
                rec.th_type_input_data = 'cancel'
                rec.th_date_input_data_approved = False

    @api.onchange('th_type_raw_material')
    def onchange_raw_material(self):
        if self.th_type_raw_material != 'approved':
            self.th_date_raw_materials_approved = False
        self.th_date_raw_materials_approved = fields.Date.today()

    @api.onchange('th_type_input_data')
    def onchange_input_data(self):
        if self.th_type_input_data != 'approved':
            self.th_date_input_data_approved = False
        self.th_date_input_data_approved = fields.Date.today()

    def th_action_check_again(self):
        for rec in self:
            rec.th_is_check_again = True

    @api.depends("project_id")
    def _compute_th_domain_partner(self):
        for rec in self:
            domain = []
            list_partner = self.env.ref('th_project_itc.th_group_project_employee').users.mapped('partner_id').ids
            if list_partner:
                domain.append(('id', 'in', list_partner))
            rec.th_domain_partner = json.dumps(domain)

    def action_th_create_task_bug(self):
        return {
            'type': 'ir.actions.act_window',
            'name': 'Tạo Bug',
            'view_mode': 'form',
            ...
            'target': 'new',
            'views': [[self.env.ref('th_project_itc.th_view_task_form2').id, 'form']],
            'context': {
                'default_parent_id': self.id,
                'default_project_id': self.project_id.id,
                'default_display_project_id': self.project_id.id,
                'default_th_is_bug': True,
                'default_partner_id': self.partner_id.id,
                'default_user_ids': [(6, 0, self.user_ids.ids)],
                'default_th_user_test_ids': [(6, 0, self.th_user_test_ids.ids)],
            }
        }

    # @api.onchange('stage_id', 'user_ids', 'th_user_test_ids')
    # def _onchange_th_date_kpi(self):
    #     for rec in self:
    #         rec.sudo().th_date_program = False if not rec.sudo().th_date_program else rec.sudo().th_date_program
    #         rec.sudo().th_date_complete_program = False if not rec.sudo().th_date_complete_program else rec.sudo().th_date_complete_program
    #         rec.sudo().th_date_test = False if not rec.sudo().th_date_test else rec.sudo().th_date_test
    #         rec.sudo().th_date_complete_test = False if not rec.sudo().th_date_complete_test else rec.sudo().th_date_complete_test
    #         today = fields.Date.today()
    #         if rec.sudo().stage_id and rec.sudo().user_ids and self.sudo().env.user.id in rec.sudo().user_ids.ids:
    #             if rec.sudo().stage_id.id == self.sudo().env.ref('th_project_itc.th_project_project_task_type_3').id:
    #                 rec.sudo().th_date_program = today
    #             if rec.sudo().stage_id.id == self.sudo().env.ref('th_project_itc.th_project_task_type_complete_stage').id:
    #                 rec.sudo().th_date_complete_program = today
    #         if rec.sudo().stage_id and rec.sudo().user_ids and self.sudo().env.user.id in rec.sudo().th_user_test_ids.ids:
    #             if rec.sudo().stage_id.id == self.sudo().env.ref('th_project_itc.th_project_project_task_type_5').id:
    #                 rec.sudo().th_date_test = today
    #             if rec.sudo().stage_id.id == self.sudo().env.ref('th_project_itc.th_project_project_task_type_8').id:
    #                 rec.sudo().th_date_complete_test = today

    @api.depends('user_ids', 'th_user_test_ids')
    def _compute_th_project_team_id(self):
        for rec in self:
            rec.th_project_team_ids = False
            if rec.user_ids or rec.th_user_test_ids:
                user_ids = rec.user_ids.ids + rec.th_user_test_ids.ids
                team = self.env['th.project.team'].search(['|', ('th_manager_id', 'in', user_ids), ('th_member_ids', 'in', user_ids)])
                rec.th_project_team_ids = [(6, 0, team.ids if team else [])]
    ...
    def _compute_th_finish_date(self):
        for task in self:
            if task.stage_id.is_completed_state:
                task.th_finish_date = fields.Datetime.today()
            else:
                task.th_finish_date =False

    # @api.depends('stage_id')
    # def _compute_th_stage_domain(self):
    #     domain = []
    #     if self._context.get('default_project_id'):
    #         test_stage5 = self.env.ref('th_project_itc.th_project_project_task_type_5')
    #         test_stage8 = self.env.ref('th_project_itc.th_project_project_task_type_8')
    #         test_stage9 = self.env.ref('th_project_itc.th_project_project_task_type_9')
    #         test_stage10 = self.env.ref('th_project_itc.th_project_project_task_type_10')
    #         ex_stage = [test_stage5.id, test_stage8.id, test_stage9.id, test_stage10.id]
    #         for rec in self:
    #             if rec.stage_id.id not in ex_stage:
    #                 status_group_ids = self.env['th.project.task.type'].search(
    #                     [('th_project_ids', '=', rec.project_id.id)])
    #                 domain.append(('id', 'in', status_group_ids.ids))
    #             else:
    #                 domain.append(('id', 'in', ex_stage))
    #     elif self._context.get('th_other_tasks'):
    #         for rec in self:
    #             other_tasks_stage_id = self.env['th.project.task.type'].search([('is_other_tasks_stage', '=', True)])
    #             domain.append(('id', 'in', other_tasks_stage_id.ids))
    #     self.th_stage_domain = json.dumps(domain)
    ...
    def _compute_personal_stage_id(self):
        ...
        personal_stages = self.env['th.project.task.stage.personal'].search([('user_id', '=', self.env.uid), ('task_id', 'in', self.ids)])
    ...
    def _populate_missing_personal_stages(self):
        ...
        personal_stages_without_stage = self.env['th.project.task.stage.personal'].sudo().search([('task_id', 'in', self.ids), ('stage_id', '=', False)])
        if personal_stages_without_stage:
            user_ids = personal_stages_without_stage.user_id
            personal_stage_by_user = defaultdict(lambda: self.env['th.project.task.stage.personal'])
            for personal_stage in personal_stages_without_stage:
                personal_stage_by_user[personal_stage.user_id] |= personal_stage
            for user_id in user_ids:
                stage = self.env['th.project.task.type'].sudo().search([('user_id', '=', user_id.id)], limit=1)
                # In the case no stages have been found, we create the default stages for the user
                if not stage:
                    stages = self.env['th.project.task.type'].sudo().with_context(lang=user_id.partner_id.lang, default_project_id=False).create(
    ...
    def _compute_recurrence_message(self):
        ...
        for task in self.filtered(lambda t: t.recurring_task and t._is_recurrence_valid()):
            ...
            recurring_dates = self.env['th.project.task.recurrence']._get_next_recurring_dates(
    ...
    def _compute_recurring_count(self):
        ...
        count = self.env['th.project.task']._read_group([('recurrence_id', 'in', recurring_tasks.recurrence_id.ids)], ['id'], 'recurrence_id')
    ...
    def _compute_dependent_tasks_count(self):
        ...
        if tasks_with_dependency:
            group_dependent = self.env['th.project.task']._read_group([
    ...
    def _get_attachments_search_domain(self):
        self.ensure_one()
        return [('res_id', '=', self.id), ('res_model', '=', 'th.project.task')]
    ...
    def _compute_stage_id(self):
        for task in self:
            if task.project_id:
                if task.project_id not in task.stage_id.th_project_ids:
    ...
    def _search_portal_user_names(self, operator, value):
        ...
        query = """
            SELECT task_user.task_id
              FROM th_project_task_user_rel task_user
    ...
    def get_empty_list_help(self, help):
        ...
        if project_id:
            name = self.env['th.project.project'].browse(project_id).label_tasks
            if name: tname = name.lower()

        self = self.with_context(
            empty_list_help_id=self.env.context.get('default_project_id'),
            empty_list_help_model='th.project.project',
    ...
    def stage_find(self, section_id, domain=[], order='sequence, id'):
        ...
        if section_ids:
            ...
            for section_id in section_ids:
                search_domain.append(('th_project_ids', '=', section_id))
        search_domain += list(domain)
        # perform search, return the first found
        return self.env['th.project.task.type'].search(search_domain, order=order, limit=1).id
        ...
        if 'partner_id' in vals and not vals['partner_id']:
            ...
            if project_id or parent_id:
                partner_id = self._get_default_partner_id(
                    project_id and self.env['th.project.project'].browse(project_id),
                    parent_id and self.env['th.project.task'].browse(parent_id)
                )
                if partner_id:
                    vals['partner_id'] = partner_id
        project_id = vals.get('project_id', self.env.context.get('default_project_id'))
        if project_id:
            project = self.env['th.project.project'].browse(project_id)
    ...
    def copy_data(self, default=None):
        ...
        if self.env.user.has_group('th_project_itc.th_group_project_user'):
            return defaults
    ...
    def _load_records_create(self, vals_list):
        projects_with_recurrence = self.env['th.project.project'].search([('allow_recurring_tasks', '=', True)])
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if project_id and not "company_id" in vals:
                vals["company_id"] = self.env["th.project.project"].browse(
                    project_id
            ...
            if rec_fields and vals.get('recurring_task') is True:
                ...
                recurrence = self.env['th.project.task.recurrence'].create(rec_values)
    ...
    def write(self, vals):
        th_old_stage = False
        if 'active' in vals and not self.env.user.has_group('th_project_itc.th_group_project_employee'):
            raise ValidationError('Bạn không có quyền lưu trữ bản ghi!')
        ...
        if 'stage_id' in vals:
            ...
            deadline_stage = self.env['th.date.deadline'].search([("th_stage", "=", self.stage_id.id), ("th_project_task_id", "=", self.id)])
            if deadline_stage:
                deadline_stage.write({'th_date_finish': fields.Date.today()})
            th_old_stage = self.stage_id
        ...
        if recurrence_update != 'this':
            ...
            tasks |= self.env['th.project.task'].search(recurrence_domain)
        ...
        for rec in self:
            if vals.get('stage_id', False) and th_old_stage:
                th_stage_done = rec.stage_id.search([('th_check_stage', '=', 'done'), ('th_project_ids', 'in', rec.project_id.ids)], limit=1, order="sequence desc")
                th_stage_up = rec.stage_id.search([('th_check_stage', '=', 'up'), ('th_project_ids', 'in', rec.project_id.ids)], limit=1, order="sequence desc")
                if rec.th_type_raw_material == 'approved' and th_stage_done.sequence > rec.stage_id.sequence:
                    raise ValidationError('Bản ghi đã được thanh toán học liệu thô không thể chuyển trạng thái xuống thấp hơn học liệu đạt!')

                if rec.th_type_input_data == 'approved' and th_stage_up.sequence > rec.stage_id.sequence:
                    raise ValidationError('Bản ghi đã được thanh toán học liệu thô không thể chuyển trạng thái xuống thấp hơn học liệu đạt!')

                if th_stage_done and th_old_stage.sequence < rec.stage_id.sequence and th_stage_done.sequence <= rec.stage_id.sequence and (
                        rec.stage_id.th_check_stage == 'done' or th_stage_done.sequence < rec.stage_id.sequence) and not rec.th_date_done_courseware:
                    rec.th_date_done_courseware = datetime.now()
                    rec.th_return_stage_done = False

                if th_stage_done and th_old_stage.sequence > rec.stage_id.sequence and rec.th_date_done_courseware and (
                        th_old_stage.th_check_stage == 'done' or th_stage_done.sequence > rec.stage_id.sequence) and rec.th_date_done_courseware:
                    rec.th_date_done_courseware = False
                    rec.th_return_stage_done = True

                if th_stage_up and th_old_stage.sequence < rec.stage_id.sequence and th_stage_up.sequence <= rec.stage_id.sequence and (
                        rec.stage_id.th_check_stage == 'up' or th_stage_up.sequence < rec.stage_id.sequence) and not rec.th_date_up_courseware:
                    rec.th_date_up_courseware = datetime.now()
                    rec.th_return_stage_up = False

                if th_stage_up and th_old_stage.sequence > rec.stage_id.sequence and rec.th_date_up_courseware and (
                        th_old_stage.th_check_stage == 'up' or th_stage_up.sequence > rec.stage_id.sequence) and rec.th_date_up_courseware:
                    rec.th_date_up_courseware = False
                    rec.th_return_stage_up = True
        ...
        if vals.get('stage_id', False) and th_old_stage:
            ...
            if vals.get('stage_id') != th_old_stage.id:
                start_stage = self.env['th.date.deadline'].search(
                    [("th_stage", "=", self.stage_id.id), ("th_project_task_id", "=", self.id)])
                if start_stage:
                    start_stage.write({'th_date_start': fields.Date.today()})
        for task in self:
    ...
    def update_date_end(self, stage_id):
        project_task_type = self.env['th.project.task.type'].browse(stage_id)
    ...
    def _compute_has_late_and_unreached_milestone(self):
        if all(not task.allow_milestones for task in self):
            ...
            return
        late_milestones = self.env['th.project.milestone'].sudo()._search([  # sudo is needed for the portal user in Project Sharing.
    ...
    def _task_message_auto_subscribe_notify(self, users_per_task):
        ...
        template_id = self.env['ir.model.data']._xmlid_to_res_id('th_project_itc.th_project_message_user_assigned', raise_if_not_found=False)
        if not template_id:
            return
        ...
        for task, users in users_per_task.items():
            ...
            for user in users:
                ...
                assignation_msg = self.env['ir.qweb']._render('th_project_itc.th_project_message_user_assigned', values, minimal_qcontext=True)
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        value = self._fields['user_ids'].convert_to_cache(updated_values.get('user_ids', []), self.env['th.project.task'], validate=False)
    ...
    def _mail_track(self, tracked_fields, initial_values):
        ...
        if depends_changes and self.allow_task_dependencies and self.user_has_groups('th_project_itc.th_group_project_task_dependencies'):
            parent_ids = self.dependent_ids
            if parent_ids:
                fields_to_ids = self.env['ir.model.fields']._get_ids('th.project.task')
                ...
                subtype = self.env['ir.model.data']._xmlid_to_res_id('th_project_itc.th_mt_task_dependency_change')
                ...
                body = self.env['ir.qweb']._render('th_project_itc.task_track_depending_tasks', {
                    'child': self,
    ...
    def _creation_subtype(self):
        return self.env.ref('th_project_itc.th_mt_task_new')

    def _track_subtype(self, init_values):
        self.ensure_one()
        mail_message_subtype_per_kanban_state = {
            'blocked': 'th_project_itc.th_mt_task_blocked',
            'done': 'th_project_itc.th_mt_task_ready',
            'normal': 'th_project_itc.th_mt_task_progress',
        }
        if 'stage_id' in init_values:
            return self.env.ref('th_project_itc.th_mt_task_stage')
    ...
    def _mail_get_message_subtypes(self):
        res = super()._mail_get_message_subtypes()
        if len(self) == 1:
            dependency_subtype = self.env.ref('th_project_itc.th_mt_task_dependency_change')
            if ((self.project_id and not self.project_id.allow_task_dependencies)\
                or (not self.project_id and not self.user_has_groups('th_project_itc.th_group_project_task_dependencies')))\
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        project_user_group_id = self.env.ref('th_project_itc.th_group_project_user').id
    ...
    def _notify_by_email_get_headers(self):
        ...
        if self.project_id:
            ...
            current_objects.insert(0, 'th.project.project-%s, ' % self.project_id.id)
    ...
    def action_assign_to_me(self):
        ...
        team = self.env['th.project.team'].sudo().search([]).filtered(lambda member: uid in member.th_member_ids.ids)
        if uid not in self.project_id.th_member_ids.ids:
            raise ValidationError(_("Bạn thông phải là người tham gia của dự án. Vui lòng liên hệ Admin!"))
        if not team:
            raise  ValidationError(_("Bạn chưa là thành viên của đội nào. Vui lòng liên hệ Admin!"))
        if len(team)>1:
            return {
                'type': 'ir.actions.act_window',
                'name': 'Xác nhận phân công',
                'view_mode': 'form',
                'res_model': 'th.task.assign',
                'target': 'new',
                'context': {'default_th_task_id': self.id},
                'views': [[self.env.ref('th_project_itc.task_assign_wizard_view_form').id, 'form']],
            }
        if team and team.th_team_type == 'qc':
            self.write({'th_user_test_ids': [(4, uid)]})
        else:
            self.write({'user_ids': [(4, uid)]})
    ...
    def _get_all_subtasks(self, depth=0):
        ...
        if not children:
            return self.env['th.project.task']
        if depth == 1:
            return children
    ...
    def action_project_sharing_view_parent_task(self):
        ...
        action['views'] = [(self.env.ref('th_project_itc.th_project_sharing_project_task_view_form').id, 'form')]
        return action
    ...
    def action_project_sharing_open_subtasks(self):
        self.ensure_one()
        subtasks = self.env['th.project.task'].search([('id', 'child_of', self.id), ('id', '!=', self.id)])
        if subtasks.project_id == self.project_id:
            action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_project_sharing_project_task_action_sub_task')
            if len(subtasks) == 1:
    ...
    def action_open_ratings(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_rating_rating_action_task')
        if self.rating_count == 1:
            action['view_mode'] = 'form'
            action['res_id'] = self.rating_ids[0].id
            action['views'] = [[self.env.ref('th_project_itc.th_rating_rating_view_form_project').id, 'form']]
            return action
        else:
            return action

    def action_stop_recurrence(self):
        tasks = self.env['th.project.task'].with_context(active_test=False).search([('recurrence_id', 'in', self.recurrence_id.ids)])
    ...
    def _rating_apply_get_default_subtype_id(self):
        return self.env['ir.model.data']._xmlid_to_res_id("th_project_itc.th_mt_task_rating")
    ...
    @api.depends('th_date_deadline_ids')
    def _compute_date_deadline(self):
        for rec in self:
            if rec.th_date_deadline_ids:
                rec.date_deadline = max(rec.th_date_deadline_ids.mapped('th_deadline_end'))
                deadline_stage = self.env['th.date.deadline'].search(
                    [("th_stage", "=", rec.stage_id.id), ("th_project_task_id", "=", rec.id)])
                if deadline_stage and deadline_stage.th_deadline_end:
                    rec.th_date_deadline = deadline_stage.th_deadline_end
                else:
                    rec.th_date_deadline = False
            else:
    ...
    def compute_date_deadline(self):
        for rec in self:
            if rec.th_date_deadline_ids:
                deadline_stage = self.env['th.date.deadline'].search([("th_stage", "=", rec.stage_id.id), ("th_project_task_id", "=", rec.id)])
                if deadline_stage and deadline_stage.th_deadline_end:
                    rec.th_date_deadline = deadline_stage.th_deadline_end
                else:
                    rec.th_date_deadline = False
...
class ProjectTags(models.Model):
    ...
    th_project_ids = fields.Many2many('th.project.project', 'th_project_project_project_tags_rel', string='Projects')
    th_task_ids = fields.Many2many('th.project.task', string='Tasks')
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    ...
    #         ['|', ('th_task_ids.project_id', '=', project_id), ('th_project_ids', 'in', project_id)]))
