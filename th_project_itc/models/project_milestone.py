...
class ProjectMilestone(models.Model):
    ...
    project_id = fields.Many2one('th.project.project', required=True, default=_get_default_project_id, ondelete='cascade')
    ...
    th_task_ids = fields.One2many('th.project.task', 'milestone_id', 'Tasks')
    ...
    th_task_count = fields.Integer('# of Tasks', compute='_compute_th_task_count', groups='th_project_itc.th_group_project_milestone')
    can_be_marked_as_done = fields.Boolean(compute='_compute_can_be_marked_as_done', groups='th_project_itc.th_group_project_milestone')
    ...
    @api.depends('th_task_ids.milestone_id')
    def _compute_th_task_count(self):
        task_read_group = self.env['th.project.task']._read_group([('milestone_id', 'in', self.ids), ('allow_milestones', '=', True)], ['milestone_id'], ['milestone_id'])
        task_count_per_milestone = {res['milestone_id'][0]: res['milestone_id_count'] for res in task_read_group}
        for milestone in self:
            milestone.th_task_count = task_count_per_milestone.get(milestone.id, 0)

    def _compute_can_be_marked_as_done(self):
        if not any(self._ids):
            for milestone in self:
                milestone.can_be_marked_as_done = not milestone.is_reached and all(milestone.th_task_ids.is_closed)
            return
        ...
        if unreached_milestones:
            task_read_group = self.env['th.project.task']._read_group(
                [('milestone_id', 'in', unreached_milestones.ids)],
                ['milestone_id', 'is_closed', 'th_task_count:count(id)'],
            ...
            for res in task_read_group:
                ...
                if res['is_closed']:
                    closed_task_count += res['th_task_count']
                else:
                    opened_task_count += res['th_task_count']
    ...
    def action_view_tasks(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('th_project_itc.th_action_view_task_from_milestone')
        action['context'] = {'default_project_id': self.project_id.id, 'default_milestone_id': self.id}
        if self.th_task_count == 1:
            action['view_mode'] = 'form'
            action['res_id'] = self.th_task_ids.id
            if 'views' in action:
