...
class ResPartner(models.Model):
    ...
    th_task_ids = fields.One2many('th.project.task', 'partner_id', string='Tasks')
    th_task_count = fields.Integer(compute='_compute_th_task_count', string='# Tasks')
    ...
    def _compute_th_task_count(self):
        ...
        task_data = self.env['th.project.task']._read_group(
            domain=[('partner_id', 'in', all_partners.ids)],
            fields=['partner_id'], groupby=['partner_id']
        )

        self.th_task_count = 0
        for group in task_data:
            partner = self.browse(group['partner_id'][0])
            while partner:
                if partner in self:
                    partner.th_task_count += group['partner_id_count']
