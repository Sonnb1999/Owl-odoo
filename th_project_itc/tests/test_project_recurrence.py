...
class TestProjectrecurrence(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env.user.groups_id += cls.env.ref('th_project_itc.th_group_project_recurring_tasks')

        cls.stage_a = cls.env['th.project.task.type'].create({'name': 'a'})
        cls.stage_b = cls.env['th.project.task.type'].create({'name': 'b'})
        cls.project_recurring = cls.env['th.project.project'].with_context({'mail_create_nolog': True}).create({
            'name': 'Recurring',
    ...
    def test_recurrence_simple(self):
        with freeze_time("2020-02-01"):
            with Form(self.env['th.project.task']) as form:
    ...
    def test_recurrence_cron_repeat_after(self):
        ...
        with freeze_time("2020-01-01"):
            form = Form(self.env['th.project.task'])
            form.name = 'test recurring task'
            ...
            self.assertEqual(self.env['th.project.task'].search_count(domain), 1)
            self.env['th.project.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['th.project.task'].search_count(domain), 1, 'no extra task should be created')
            self.assertEqual(task.recurrence_id.recurrence_left, 2)

        with freeze_time("2020-01-15"):
            self.assertEqual(self.env['th.project.task'].search_count(domain), 1)
            self.env['th.project.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['th.project.task'].search_count(domain), 2)
            self.assertEqual(task.recurrence_id.recurrence_left, 1)

        with freeze_time("2020-02-15"):
            self.env['th.project.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['th.project.task'].search_count(domain), 3)
    ...
    def test_recurrence_cron_repeat_forever(self):
        ...
        with freeze_time("2020-03-15"):
            self.env['th.project.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['th.project.task'].search_count(domain), 4)
    ...
    def test_recurrence_update_task(self):
        with freeze_time("2020-01-01"):
            task = self.env['th.project.task'].create({
        ...
        task_c, task_b, task_a = self.env['th.project.task'].search([('project_id', '=', self.project_recurring.id)])
    ...
    def test_recurrence_week_day(self):
        with self.assertRaises(ValidationError), self.cr.savepoint():
            self.env['th.project.task'].create({
                'name': 'test recurring task',
                'project_id': self.project_recurring.id,
                'recurring_task': True,
                'repeat_interval': 1,
    ...
    def test_recurrence_next_dates_week(self):
        dates = self.env['th.project.task.recurrence']._get_next_recurring_dates(
    ...
    def test_recurrence_cron_repeat_after_subtasks(self):
        ...
        parent_task = self.env['th.project.task'].create({
            'name': 'Parent Task',
            'project_id': self.project_recurring.id
        })
        child_task = self.env['th.project.task'].create({
            'name': 'Child Task',
        ...
        with freeze_time("2020-01-01"):
            ...
            self.env['th.project.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['th.project.task'].search_count(domain), 2, 'no extra task should be created')
    ...
    def test_recurrence_cron_repeat_after_subsubtasks(self):
        ...
        domain = [('project_id', '=', self.project_recurring.id)]
        child_task_1, child_task_2_recurrence = self.env['th.project.task'].create([
            {'name': 'Child task 1'},
        ...
        grand_child_task_1 = self.env['th.project.task'].create({
            'name': 'Grandchild task 1 (recurrent)',
        })
        grand_child_task_2_recurrence = self.env['th.project.task'].create({
