...
export class ThProjectControlPanel extends ControlPanel {
    ...
    async _loadWidgetData() {
        if (this.show_project_update) {
            this.data = await this.rpc({
                model: 'th.project.project',
                method: 'get_last_update_or_default',
                args: [this.project_id],
            });
            this.is_project_user = await session.user_has_group('th_project_itc.th_group_project_user');
        }
    }
    ...
    async onStatusClick(ev) {
        ...
        await this.trigger('do-action', {
            action: "th_project_itc.th_project_update_all_action",
            options: {
                additional_context: {
