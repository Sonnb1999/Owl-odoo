...
class ProjectCollaborator(models.Model):
    _name = 'th.project.collaborator'
    _description = 'Collaborators in project shared'

    project_id = fields.Many2one('th.project.project', 'Project Shared', domain=[('privacy_visibility', '=', 'portal')], required=True, readonly=True)
    ...
    def create(self, vals_list):
        collaborator = self.env['th.project.collaborator'].search([], limit=1)
    ...
    def _toggle_project_sharing_portal_rules(self, active):
        ...
        task_portal_ir_rule = self.env.ref('th_project_itc.project_task_rule_portal_project_sharing').sudo()
