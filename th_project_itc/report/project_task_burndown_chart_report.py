...
class ReportProjectTaskBurndownChart(models.AbstractModel):
    _name = 'th.project.task.burndown.chart.report'
    ...
    display_project_id = fields.Many2one('th.project.project', readonly=True)
    ...
    project_id = fields.Many2one('th.project.project', readonly=True)
    stage_id = fields.Many2one('th.project.task.type', readonly=True)
    ...
    def _get_group_by_SQL(self, task_specific_domain, count_field, select_terms, from_clause, where_clause,
        ...
        project_task_query = self.env['th.project.task']._where_calc(task_specific_domain)
        ...
        field_id = IrModelFieldsSudo.search([('name', '=', 'stage_id'), ('model', '=', 'th.project.task')]).id
                                                ...
                                                LEFT JOIN (
                                                    mail_message mm
                                                        JOIN mail_tracking_value mtv ON mm.id = mtv.mail_message_id
                                                                                     ...
                                                                                     AND mm.model='th.project.task'
                                                                                     AND mm.message_type = 'notification'
                                                        JOIN project_task_type ptt ON ptt.id = mtv.old_value_integer
                                                ) ON mm.res_id = pt.id
                                      WHERE pt.active=true AND pt.id IN (SELECT id from th_task_ids)
    ...
    def _read_group_raw(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        self.env['th.project.task']._flush_search(task_specific_domain, fields=self.task_specific_fields)
