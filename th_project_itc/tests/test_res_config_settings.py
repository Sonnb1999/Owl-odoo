...
class TestResConfigSettings(TransactionCase):
    ...
    def test_set_default_analytic_plan(self):
        ...
        with Form(settings_company_1) as form:
            self.assertEqual(form.th_analytic_plan_id, self.analytic_plan_1)
            form.th_analytic_plan_id = self.analytic_plan_2
            form.save()
            self.assertEqual(settings_company_1.th_analytic_plan_id, self.analytic_plan_2)
        settings_company_2 = self.env['res.config.settings'].with_company(self.company_2).create({})
        with Form(settings_company_2) as form:
            self.assertNotEqual(form.th_analytic_plan_id, self.analytic_plan_1)
            self.assertNotEqual(form.th_analytic_plan_id, self.analytic_plan_2)
            self.assertEqual(form.th_analytic_plan_id.name, "Default")
