...
class TestProjectSubtasks(TestProjectCommon):
    ...
    def test_task_display_project_with_default_form(self):
        ...
        """
        with Form(self.env['th.project.task'].with_context({'tracking_disable': True})) as task_form:
        ...
        with Form(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_pigs.id})) as task_form:
    ...
    def test_task_display_project_with_task_form2(self):
        ...
        """
        with Form(self.env['th.project.task'].with_context({'tracking_disable': True}), view="th_project_itc.th_view_task_form2") as task_form:
            task_form.name = 'Test Task 1'
            task_form.project_id = self.project_pigs
        task = task_form.save()
        ...
        with Form(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_pigs.id}), view="th_project_itc.th_view_task_form2") as task_form:
            task_form.name = 'Test Task 2'
        task = task_form.save()
    ...
    def test_task_display_project_with_quick_create_task_form(self):
        ...
        """
        with Form(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_pigs.id}), view="th_project_itc.th_quick_create_task_form") as task_form:
            task_form.name = 'Test Task 2'
        task = task_form.save()
    ...
    def test_task_display_project_with_any_task_form(self):
        ...
        """
        form_views = self.env['ir.ui.view'].search([('model', '=', 'th.project.task'), ('type', '=', 'form')])
        for form_view in form_views:
            task_form = Form(self.env['th.project.task'].with_context({'tracking_disable': True, 'default_project_id': self.project_pigs.id, 'default_name': 'Test Task 1'}), view=form_view)
    ...
    def test_subtask_display_project(self):
        ...
        """
        # 1)
        test_subtask_1 = self.env['th.project.task'].create({
            'name': 'Test Subtask 1',
        })
        ...
        # 3)
        with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
            task_form.child_ids[0].display_project_id = self.env['th.project.project']
        ...
        with self.debug_mode():
            ...
            with Form(self.task_1.child_ids.with_context({'tracking_disable': True})) as subtask_form:
                subtask_form.parent_id = self.env['th.project.task']
            ...
            with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
                ...
                task_form.child_ids[0].display_project_id = self.project_goats
            with Form(self.task_1.child_ids.with_context({'tracking_disable': True})) as subtask_form:
                subtask_form.display_project_id = self.env['th.project.project']
                subtask_form.parent_id = self.env['th.project.task']
            orphan_subtask = subtask_form.save()
    ...
    def test_subtask_stage(self):
        ...
        """
        stage_a = self.env['th.project.task.type'].create({'name': 'a', 'sequence': 1})
        stage_b = self.env['th.project.task.type'].create({'name': 'b', 'sequence': 10})
        ...
        test_subtask_2 = self.env['th.project.task'].create({
            'name': 'Test Subtask 2',
        })
        with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
    ...
    def test_copy_project_with_subtasks(self):
        self.env['th.project.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Parent Task',
    ...
    def test_subtask_creation_with_form(self):
        ...
        with Form(child_subtask.with_context(tracking_disable=True)) as subtask_form:
            ...
            subtask_form.child_ids[0].display_project_id = self.env['th.project.project']
